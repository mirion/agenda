...
var Agenda = module.exports = function(config, cb) {
  ...
  this._allowMongoPre26 = config.allowMongoPre26;
  if (config.mongo) {
...
Agenda.prototype.jobs = function( query, cb ){
  var self = this;
  this._collection.find( adaptQueryForId( query ) ).toArray( function( error, result ){
    var jobs;
    if( !error ){
...
Agenda.prototype.cancel = function(query, cb) {
  this._collection.deleteMany( adaptQueryForId( query ), function( error, result ){
    if (cb) {
...
Agenda.prototype.saveJob = function(job, cb) {
  ...
  if (id) {
    this._collection.findAndModify(adaptQueryForId({_id: id}), {}, update, {new: true}, processDbResult );
    ...
    this._collection.findAndModify(adaptQueryForId(query), {}, update, {upsert: true, new: true}, processDbResult);
  } else {
...
function processJobs(extraJob) {
  ...
  if (!extraJob) {
    ...
    var now = new Date();
    self._collection.findAndModify(adaptQueryForId({ _id: extraJob.attrs._id, lockedAt: null, disabled: { $ne: true } }), {}, { $set: { lockedAt: now } }, { new: true }, function(err, resp) {
...
function adaptQueryForId( query )
{
  if( query && query._id )
  {
